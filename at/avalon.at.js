 ...
 * @enName at
 * @description
 *    经常使用微博的人会发现，当我们在输入框输入@然后敲一个人的名字，会弹出一个<code>tip提示层</code>，里面是一个名字列表。这是社交网站或应用最近非常流行的功能。
...
define(["avalon", "text!./avalon.at.html", "css!../chameleon/oniui-common.css", "css!./avalon.at.css"], function(avalon, template) {
    ...
    widget.defaults = {
        ...
        at: "@",
        /**
         @param datalist {[]} 字符串数组，不可监控，(名字取自HTML的datalist同名元素)
         */
        datalist: [],
        _datalist: [], //实际是应用于模板上的字符串数组，它里面的字符可能做了高亮处理
        /**
         @param template {""} 弹出层的模板，如果为空，使用默认模板，注意要在上面添加点击或hover处理
         */
        template: "",
